(function(){
  ...
  ready(async function(){
    try{
      ...
      const invCount = el('invCount'), invLimit = el('invLimit'); // peuvent ne pas exister
      ...
      function updateInvCount(){
        if (!grid) return;
        const used = Array.from(grid.children).filter(c=>c.textContent && c.textContent.trim()).length;
